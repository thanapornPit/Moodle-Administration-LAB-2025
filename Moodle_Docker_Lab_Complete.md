...
<img width="1032" height="206" alt="image" src="https://github.com/user-attachments/assets/6a8e6c5c-8f0c-4cde-b6ca-9c6e8c9ad61e" />
...
<img width="1036" height="487" alt="image" src="https://github.com/user-attachments/assets/11f0040e-f603-4a35-a805-5a307d2ed6d8" />
...
<img width="1042" height="682" alt="image" src="https://github.com/user-attachments/assets/fd3c8a7a-68a0-412a-a89a-9b7c400872f2" />
...
<img width="1908" height="851" alt="image" src="https://github.com/user-attachments/assets/0df75329-de29-456b-a169-b615d1f30adc" />
...
<img width="1917" height="835" alt="image" src="https://github.com/user-attachments/assets/014b96e4-1a8f-477a-85ec-624386a36e1c" />
...
<img width="1832" height="873" alt="image" src="https://github.com/user-attachments/assets/795c71e7-f4e1-4e19-9a20-28cff9f1b522" />
...
<img width="1897" height="821" alt="image" src="https://github.com/user-attachments/assets/3a009564-34dd-4aa4-a719-4afd1b61172f" />
...
<img width="1918" height="865" alt="image" src="https://github.com/user-attachments/assets/90acf43f-8ee6-4a47-b4b6-139fcb4f19b6" />
...
<img width="1902" height="767" alt="image" src="https://github.com/user-attachments/assets/540c50b4-a8f1-45a3-bd35-98af4ab6b739" />
...
<img width="1897" height="855" alt="image" src="https://github.com/user-attachments/assets/f1528cc8-842d-41ce-b124-f2d7590f5be1" />
...
คำตอบ:
```
Docker Image คือ ไฟล์ต้นแบบที่ใช้สำหรับสร้าง Container ภายในจะบรรจุโปรแกรม ระบบปฏิบัติการ และการตั้งค่าต่าง ๆ เปรียบเสมือน “แม่พิมพ์”
...
คำตอบ:
```
มี 2 Container ได้แก่ moodle_app ทำหน้าที่เป็น Web Application Server สำหรับให้บริการระบบ Moodle

db (MariaDB) ทำหน้าที่เป็น Database Server สำหรับเก็บข้อมูล เช่น ผู้ใช้ รายวิชา และคะแนน
...
คำตอบ:
```
ใช้ Docker Volume แบบ Persistent Volume

ข้อดี ข้อมูลไม่หายเมื่อ Container ถูกลบ สำรองข้อมูลได้ง่าย สามารถใช้ข้อมูลร่วมกันได้

ข้อเสีย

ใช้พื้นที่เก็บข้อมูลเพิ่ม
...
คำตอบ:
```
Network ทำหน้าที่เชื่อมต่อ Container ให้สามารถติดต่อสื่อสารกันได้ Container สื่อสารกันผ่านชื่อ Service เช่น

moodle_app เชื่อมต่อฐานข้อมูลผ่านชื่อ db เปรียบเหมือนอยู่ในเครือข่ายเดียวกันและเรียกหากันด้วยชื่อเครื่อง
...
คำตอบ:
```
depends_on ใช้กำหนดลำดับการเริ่มทำงานของ Container เช่น ให้ Database เริ่มทำงานก่อน Moodle ช่วยลดปัญหา Moodle เชื่อมต่อฐานข้อมูลไม่สำเร็จตอนเริ่มต้นระบบ
```
...
คำตอบ:
```
ต้องแก้ไขส่วน ports ใน service moodle_app ตัวอย่าง

ports:
  - "9000:80"


หมายถึง เข้าผ่าน browser ด้วย http://localhost:9000
...
คำตอบ:
```
หมายถึง ให้ Moodle เชื่อมต่อ Database ที่ Container ชื่อ db ไม่ใช้ localhost เพราะ localhost หมายถึง Container ตัวเอง ไม่ใช่ Container ฐานข้อมูล Docker Compose อนุญาตให้เรียก Container ด้วยชื่อ Service ได้โดยตรง
```
...
ข้อดี ติดตั้งง่าย ย้ายระบบได้สะดวก ควบคุมเวอร์ชันได้ ลดปัญหาความเข้ากันของระบบ

ข้อเสีย ต้องเรียนรู้ Docker เพิ่ม ใช้ทรัพยากรเครื่องเพิ่มขึ้น
...
ข้อดี ควบคุมระบบได้ละเอียด ใช้ทรัพยากรน้อยกว่า

ข้อเสีย ติดตั้งยาก อัปเดตและย้ายระบบลำบาก
```
...
ตรวจสอบค่าการตั้งค่า Environment Variables


วิธีแก้ไข:
ตรวจสอบชื่อ Database host ให้ถูกต้อง ตรวจสอบว่า Container db ทำงานอยู่ ,Restart Container ใหม่ docker-compose restart
...
คำตอบ:
```
คำสั่งนี้จะ หยุดและลบ Container ลบ Network ลบ Volume

ผลลัพธ์คือ ข้อมูลทั้งหมดใน Database และ Moodle จะถูกลบถาวร
